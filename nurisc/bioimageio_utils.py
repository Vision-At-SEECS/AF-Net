...
//*******************************************************************
// Date: July-2021
// Credits: nurisc, DeepImageJ
// URL:
//      https://github.com/nurisc/nurisc
//      https://deepimagej.github.io/deepimagej
// This macro was adapted from
// https://github.com/deepimagej/imagej-macros/blob/648caa867f6ccb459649d4d3799efa1e2e0c5204/nurisc2D_Post-processing.ijm
// Please cite the respective contributions when using this code.
//*******************************************************************
//  Macro to run nurisc postprocessing on segmentation images.
//  nurisc and deepImageJ plugins need to be installed.
...
// Run nurisc plugin
run("Command From Macro", "command=[de.csbdresden.nurisc.nurisc2DNMS], args=['prob':'scores', 'dist':'distances', 'probThresh':'" + probThresh + "', 'nmsThresh':'" + nmsThresh + "', 'outputType':'Both', 'excludeBoundary':'2', 'roiPosition':'Stack', 'verbose':'false'], process=[false]");
"""
    ...
    try:
        ...
        if error:
            raise RuntimeError(
                ...
                "Please install nurisc as follows: pip install 'nurisc[bioimageio]'\n"
                "(You do not need to uninstall nurisc first.)"
            )
        else:
            return None
...
def _create_nurisc_dependencies(outdir):
    from ruamel.yaml import YAML
    from tensorflow import __version__ as tf_version
    from . import __version__ as nurisc_version
    pkg_info = get_distribution("nurisc")
    ...
    # only nurisc and tensorflow as pip dependencies
    tf_major, tf_minor = LooseVersion(tf_version).version[:2]
    reqs_pip = (f"nurisc>={nurisc_version}", f"tensorflow>={tf_major}.{tf_minor},<{tf_major+1}")
    # conda environment
    env = dict(
        name = 'nurisc',
...
def _create_nurisc_doc(outdir):
    doc_path = outdir / "README.md"
    text = (
        "# nurisc Model\n"
        "This is a model for object detection with star-convex shapes.\n"
        "Please see the [nurisc repository](https://github.com/nurisc/nurisc) for details."
    )
...
def _get_nurisc_metadata(outdir, model):
    metadata, *_ = _import()
    package_data = metadata("nurisc")
    ...
    data = dict(
        ...
        dependencies=_create_nurisc_dependencies(outdir),
        ...
        covers=["https://raw.githubusercontent.com/nurisc/nurisc/master/images/nurisc_logo.jpg"],
        documentation=_create_nurisc_doc(outdir),
    )
    return data
...
def _get_weights_and_model_metadata(outdir, model, test_input, test_input_axes, test_input_norm_axes, mode, min_percentile, max_percentile):
    ...
    weights_file = outdir / "nurisc_weights.h5"
    ...
    if is_2D:
        macro_file = outdir / "nurisc_postprocessing.ijm"
        ...
        config['nurisc'].update(postprocessing_macro=macro_file.name)
...
def export_bioimageio(
    ...
    """Export nurisc model into bioimage.io format, https://github.com/bioimage-io/spec-bioimage-io.

    Parameters
    ----------
    model: nurisc2D, nurisc3D
    ...
    test_input_norm_axes: str
        the axes of the test input which will be jointly normalized, for example 'ZYX' for all spatial dimensions ('Z' ignored for segmentation input)
    ...
    from .models import nurisc2D, nurisc3D
    isinstance(model, (nurisc2D, nurisc3D)) or _raise(ValueError("not a valid model"))
    ...
    with tempfile.TemporaryDirectory() as _tmp_dir:
        ...
        kwargs = _get_nurisc_metadata(tmp_dir, model)
...
def import_bioimageio(source, outpath):
    """Import nurisc model from bioimage.io format, https://github.com/bioimage-io/spec-bioimage-io.

    Load a model in bioimage.io format from the given `source` (e.g. path to zip file, URL)
    and convert it to a regular nurisc model, which will be saved in the folder `outpath`.
    ...
    outpath: str, Path
        folder to save the nurisc model (must not exist previously)

    Returns
    -------
    nurisc2D or nurisc3D
        nurisc model loaded from `outpath`
    ...
    *_, bioimageio_core, _ = _import()
    ...
    with tempfile.TemporaryDirectory() as _tmp_dir:
        ...
        # read the nurisc specific content
        'nurisc' in biomodel.config or _raise(RuntimeError("bioimage.io model not compatible"))
        config = biomodel.config['nurisc']['config']
        thresholds = biomodel.config['nurisc']['thresholds']
        weights = biomodel.config['nurisc']['weights']
        ...
        # save the config and threshold to json, and weights to hdf5 to enable loading as nurisc model
    ...
    model_class = (nurisc2D if config['n_dim'] == 2 else nurisc3D)
