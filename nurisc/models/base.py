...
#       previous segmentation behavior was norm_by_mask=False
#       same question for reg_weight? use 1e-4 (as in 3D) or 0 (as in segmentation)?
...
# Compounded Loss Dice+ Categorical Cross Entropy
...
# Compound Tversky + Categorical Cross Entropy Loss
def compound_tversky_cce(weights, ndim, alpha=0.7, gamma=1.33):
    """ sum of weighted cce and dice loss """
    _cce = weighted_categorical_crossentropy(weights, ndim, gamma)
...
class nuriscDataBase(RollingSequence):
...
class nuriscBase(BaseModel):
    ...
    def __init__(self, config, name=None, basedir='.'):
        ...
        print("Using default values: prob_thresh={prob:g}, nms_thresh={nms:g}.".format(prob=self.thresholds.prob,
    ...
    def prepare_for_training(self, optimizer=None):
        ...
        if self._is_multiclass():
            if self.config.train_class_loss == 'cce':
                ...
                prob_class_loss = compound_dice_cce(self.config.train_class_weights, ndim=self.config.n_dim,
                                                    gamma=self.config.train_focal_gamma)
            elif self.config.train_class_loss == 'cce_tversky':
                prob_class_loss = compound_tversky_cce(self.config.train_class_weights, ndim=self.config.n_dim,
                                                       gamma=self.config.train_focal_gamma)
            else:
    ...
    def _predict_setup(self, img, axes, normalizer, n_tiles, show_tile_progress, predict_kwargs):
        ...
        resizer = nuriscPadAndCropResizer(grid=grid_dict)
    ...
    def predict_sparse(self, img, prob_thresh=None, axes=None, normalizer=None, n_tiles=None, show_tile_progress=True,
                       b=2, **predict_kwargs):
...
class nuriscPadAndCropResizer(Resizer):
