...
Input, Conv2D, MaxPooling2D, Concatenate, UpSampling2D = keras_import('layers', 'Input', 'Conv2D', 'MaxPooling2D',
                                                                      'Concatenate', 'UpSampling2D')
Model = keras_import('models', 'Model')

from .base import nuriscBase, nuriscDataBase, _tf_version_at_least
...
class nuriscData2D(nuriscDataBase):
    ...
    def __getitem__(self, i):
        ...
        if self.shape_completion:
            ...
            _dist = np.stack(
                [star_dist(lbl, self.n_rays, mode=self.sd_mode)[self.b + (slice(None),)] for lbl in Y_cleared])
        ...
        if self.n_classes is None:
            ...
            prob_class = np.stack(
                tuple((mask_to_categorical(y, self.n_classes, self.classes[k]) for y, k in zip(Y, idx))))
...
class Config2D(BaseConfig):
    ...
    train_dist_loss : str
        Training loss for generated nuclei distances ('mse' or 'mae').
    ...
    def __init__(self, axes='YX', n_rays=36, n_channel_in=1, grid=(1, 1), n_classes=None, backbone='unet', **kwargs):
        """See class docstring."""
...
class nurisc2D(nuriscBase):
    ...
    def _build(self):
        ...
        output_prob = Conv2D(1, (1, 1), name='prob', padding='same', activation='sigmoid')(
            _head(unet, self.config.unet_n_filter_base // 2))

        output_dist = Conv2D(self.config.n_rays, (1, 1), name='dist', padding='same', activation='linear')(
        ...
        if self._is_multiclass():
            ...
            output_prob_class = Conv2D(self.config.n_classes + 1, (1, 1), name='prob_class', padding='same',
                                       activation='softmax')(_head(unet_class, self.config.unet_n_filter_base // 2))
    ...
    def train(self, X, Y, validation_data, classes='auto', augmenter=None, seed=None, epochs=None, steps_per_epoch=None,
        ...
        [(p - 2 * b) % d == 0 or _raise(ValueError(
            "'train_patch_size' - 2*'train_completion_crop' must be divisible by {d} along axis '{a}'".format(a=a,
                                                                                                              d=d) if self.config.train_shape_completion else
        ...
        _data_val = nuriscData2D(validation_data[0], validation_data[1], classes=classes_val, batch_size=n_take,
                                   length=1, shuffle=False, **data_kwargs)
        data_val = _data_val[0]

        data_train_nurisc = nuriscData2D(X, Y, classes=classes,
                                             batch_size=1,
                                             augmenter=augmenter,
                                             length=self.config.train_batch_size * epochs * steps_per_epoch,
                                             **data_kwargs)

        self.data_train = wrap_nuriscdata_as_tfdata(data_train_nurisc,
    ...
    def _instances_from_prediction(self, img_shape, prob, dist, points=None, prob_class=None, prob_thresh=None,
                                   nms_thresh=None, overlap_label=None, return_labels=True, scale=None, **nms_kwargs):
        """
        ...
        if overlap_label is not None: raise NotImplementedError("overlap_label not supported for segmentation yet!")
        ...
        res_dict = dict(coord=coord, points=points, prob=probi, nms=None)
    ...
    def _axes_div_by(self, query_axes):
        self.config.backbone in ('unet', 'ffnet', 'fanet') or _raise(NotImplementedError())
