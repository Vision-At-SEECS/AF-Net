...
def wrap_nuriscdata_as_tfdata(
    ...
    """
    wrap a nurisc data generator (whose batchsize needs to be 1) into a tf dataset

    nurisc data generator data return values:
    ...
    if not data.batch_size == 1:
        raise ValueError("Can only convert nurisc data generator with batchsize 1 to tf data!")
    ...
    def _pre_map(idx):
        ...
        if verbose:
            print(f"fetching nurisc item {idx:4d}   ({data.data_size})")
        a, b = data[idx]
