...
Command line script to perform prediction in segmentation
...
Prediction script for a segmentation nurisc model, usage: nurisc-predict -i input.tif -m model_folder_or_pretrained_name -o output_folder

""")
    parser.add_argument("-i","--input", type=str, nargs="+", required=True, help = "input file (tiff)")
    parser.add_argument("-o","--outdir", type=str, default='.', help = "output directory")
    parser.add_argument("--outname", type=str, nargs="+", default='{img}.nurisc.tif', help = "output file name (tiff)")
    ...
    from nurisc.models import nurisc2D
    from imageio import imread
    from tifffile import imwrite

    get_registered_models(nurisc2D, verbose=True)

    if pathlib.Path(args.model).is_dir():
        model = nurisc2D(None, name=args.model)
    else:
        model = nurisc2D.from_pretrained(args.model)
