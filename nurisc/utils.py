...
try:
    ...
    _edt_parallel = os.environ.get('nurisc_EDT_NUM_THREADS', _edt_parallel_default)
    try:
        _edt_parallel = min(_edt_parallel_max, int(_edt_parallel))
    except ValueError as e:
        warnings.warn(f"Invalid value ({_edt_parallel}) for nurisc_EDT_NUM_THREADS. Using default value ({_edt_parallel_default}) instead.")
