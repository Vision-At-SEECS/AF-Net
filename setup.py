...
with open(path.join(_dir,'nurisc','version.py'), encoding="utf-8") as f:
    exec(f.read())
...
external_root = path.join(_dir, 'nurisc', 'lib', 'external')
...
setup(
    ...
    ext_modules=[
        Extension(
            'nurisc.lib.stardist2d',
            sources = ['nurisc/lib/stardist2d.cpp', 'nurisc/lib/utils.cpp'] + clipper_src,
            extra_compile_args = ['-std=c++11'],
            include_dirs = get_numpy_include_dirs() + [clipper_root, nanoflann_root],
        ),
        Extension(
            'nurisc.lib.stardist3d',
            sources = ['nurisc/lib/stardist3d.cpp', 'nurisc/lib/stardist3d_impl.cpp', 'nurisc/lib/utils.cpp'] + qhull_src,
            extra_compile_args = ['-std=c++11'],
            include_dirs = get_numpy_include_dirs() + [qhull_root, nanoflann_root],
        ),
    ],

    package_data={'nurisc': [ 'kernels/*.cl', 'data/images/*' ]},
    ...
    entry_points = {
        'console_scripts': [
            'nurisc-predict2d = nurisc.scripts.predict2d:main',
            'nurisc-predict3d = nurisc.scripts.predict3d:main',
        ],
    }
